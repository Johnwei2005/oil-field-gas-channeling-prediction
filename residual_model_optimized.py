...
class PhysicsModel:
    ...
    def fit(self, X, y):
        ...
        if 'permeability' in features:
            self.coefficients['permeability'] = 0.1
        if 'oil_viscosity' in features:
            self.coefficients['oil_viscosity'] = -0.05
        if 'well_spacing' in features:
            self.coefficients['well_spacing'] = 0.02
        if 'effective_thickness' in features:
            self.coefficients['effective_thickness'] = 0.03
        if 'formation_pressure' in features:
            self.coefficients['formation_pressure'] = 0.01
        if 'mobility_ratio' in features:
            self.coefficients['mobility_ratio'] = 0.15
        if 'fingering_index' in features:
            self.coefficients['fingering_index'] = 0.2
        if 'flow_capacity_index' in features:
            self.coefficients['flow_capacity_index'] = 0.25
        if 'gravity_number' in features:
            self.coefficients['gravity_number'] = 0.1
        if 'pressure_viscosity_ratio' in features:
            self.coefficients['pressure_viscosity_ratio'] = 0.05
        
        # 设置截距
        self.intercept = np.mean(y) * 0.5
