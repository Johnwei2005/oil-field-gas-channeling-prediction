...
def run_model_optimization():
    ...
    for method in methods:
        ...
        X = df_physics[features].drop(columns=[target_col])
        y = df_physics[features][target_col]
    ...
    logger.info(f"最佳特征集 (共{len(best_features)-1}个): {best_features}")
    
    # 使用最佳特征集
    selected_features = best_features
    df_selected = df_physics[selected_features]
    ...
    if best_r2 >= target_r2:
        logger.info(f"模型优化成功! 最佳R²: {best_r2:.4f} >= 目标R²: {target_r2}")
    else:
        logger.warning(f"模型优化未达到目标! 最佳R²: {best_r2:.4f} < 目标R²: {target_r2}")
    
    return results[best_model_type]['test']['final_r2'] >= target_r2, best_model_type, results_summary

if __name__ == "__main__":
    success, best_model_type, results_summary = run_model_optimization()
    
    if success:
        print(f"模型优化成功! 最佳模型: {best_model_type}, R²: {results_summary['best_r2']:.4f}")
    else:
        print(f"模型优化未达到目标! 最佳模型: {best_model_type}, R²: {results_summary['best_r2']:.4f}")
